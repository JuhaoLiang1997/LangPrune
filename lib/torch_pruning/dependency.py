...
class Group(object):
    ...
    def to_dict(self):
        """
        Convert group info to dict for JSON serialization
        """
        group_info = []
        for i, (dep, idxs) in enumerate(self._group):
            module_name = dep.target.module.__class__.__name__  # Get module class name
            module_str = str(dep)  # Full string description of dep
