...
from lib.utils.prompter import Prompter, ZeroPrompter
...
def main(args):
    ...
    def generate_and_tokenize_prompt(data_point):
        ...
        elif 'wikipedia' in args.data_path.lower():
    ...
    if args.multilingual_train:
        if args.cache_dataset and os.path.exists('./dataset/cache/wikipedia_{}.bin'.format("_".join(args.multilingual_language))):
            preprocess_data = torch.load('./dataset/cache/{}.bin'.format("_".join(args.multilingual_language)))
            train_data, val_data = preprocess_data['train'], preprocess_data['val']
        else:
            all_train = []
            all_val = []
            for lang in args.multilingual_language:
                ###### Version3
                print("Loading data for language: {}".format(lang))
                data = load_dataset("./dataset/wikipedia/20231101.{}".format(lang), split='train')

                train_val = data.train_test_split(
                    test_size=args.val_set_size, shuffle=True, seed=42)
                
                training_data = train_val["train"].select(range(args.num_language_samples))
                eval_data = train_val["test"]

                all_train.append(training_data)
                all_val.append(eval_data)
            
            train_data = concatenate_datasets(all_train).shuffle(seed=42).map(generate_and_tokenize_prompt, num_proc=50)
            val_data = concatenate_datasets(all_val).shuffle(seed=42).map(generate_and_tokenize_prompt, num_proc=50)

            if args.cache_dataset and args.local_rank == 0:
                cache_file = './dataset/cache/wikipedia_{}.bin'.format("_".join(args.multilingual_language))
                Path(cache_file).parent.mkdir(parents=True, exist_ok=True)
                torch.save({"train": train_data, "val": val_data}, cache_file)
            
            if torch.distributed.is_initialized():
                torch.distributed.barrier()
    elif args.multi_task_train:
        pass
    else:
        data = load_dataset(args.data_path)
        if args.cache_dataset and os.path.exists('./dataset/cache/{}.bin'.format(args.data_path)):
            preprocess_data = torch.load('./dataset/cache/{}.bin'.format(args.data_path))
            train_data, val_data = preprocess_data['train'], preprocess_data['val']
        else:
            ...
            if args.cache_dataset and args.local_rank == 0:
                cache_file = './dataset/cache/{}.bin'.format(args.data_path)
...
if __name__ == "__main__":
    ...
    # multilingual data
    parser.add_argument('--multilingual_train', action='store_true', default=False, help="Whether to use multilingual data for training.")
    # total_samples = num_language_samples * len(multilingual_language)
    
    parser.add_argument('--multilingual_language', type=str, nargs='+', default=["ar", "cs", "ru", "de", "en", "es", "id", "zh"], help='multilingual data name')
    parser.add_argument('--num_language_samples', type=int, default=2000, help='number of samples for each language per epoch')
    # parser.add_argument('--max_length', type=int, default=256, help='max length of each sample for multilingual data')

    parser.add_argument('--multi_task_train', action='store_true', default=False, help="Whether to use multilingual data for training.")
    parser.add_argument('--multi_task', type=str, nargs='+', default=["ar", "cs", "ru", "de", "en", "es", "id", "zh"], help='multilingual data name')

    parser.add_argument('--CUDA_VISIBLE_DEVICES', type=str, default='0,1,2,3', help='visible gpus')
